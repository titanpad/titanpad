...
function render500(ex) {
  ...
  var trace = null;
  if (ex && (!isProduction())) {
    trace = exceptionutils.getStackTraceHTML(ex);
  }
...
function httpsHost(h) {
  ...
  if (appjet.config.listenSecurePort != "443" && !appjet.config.assumePortsAreDefaults) {
...
function httpHost(h) {
  h = h.split(":")[0];  // strip any existing port
  if (appjet.config.listenPort != "80" && !appjet.config.assumePortsAreDefaults) {
